...
func GetPaginationLinks(config *AppConfig, params PaginationParams) (PaginationLinks, error) {
	...
	for i := 1; int32(i) <= totalPages; i++ {
		links = append(links, PageLink{
			Page:          int32(i),
			Url:           fmt.Sprintf("%s/%s?pages=%s", config.AppURL, params.Path, fmt.Sprint(i)),
	...
	return PaginationLinks{
		CurrentPage: fmt.Sprintf("%s/%s?pages=%s", config.AppURL, params.Path, fmt.Sprint(params.CurrentPage)),
		NextPage:    fmt.Sprintf("%s/%s?pages=%s", config.AppURL, params.Path, fmt.Sprint(nextPage)),
		PrevPage:    fmt.Sprintf("%s/%s?pages=%s", config.AppURL, params.Path, fmt.Sprint(prevPage)),
